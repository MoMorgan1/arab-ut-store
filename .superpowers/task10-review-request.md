...
BRIEF REVIEW ONLY. Do NOT implement anything. Do NOT create or modify any
repository file. You are reviewing an implementation brief written by the
orchestrator for a later Gemini worker run. Verify every technical claim
against this repository's actual code (contracts, config accessors, test
style, workflow file) and against the canonical plan section it cites.
Return: (1) verdict READY or AMEND; (2) a numbered list of required
amendments with exact replacement wording where possible; (3) optional
suggestions clearly separated. Keep it concise.
...
1. Preserve existing contracts exactly:
   ...
   accessors (model fixed to gpt-5.6-luna, request_timeout_seconds default 30,
...
3. Request body POSTed to https://api.openai.com/v1/responses is EXACTLY:
   ...
   Validate before sending: nonempty key, safety_identifier must be 64
   lowercase hex chars, messages a nonempty bounded list of role/content pairs.
...
6. Exhaustive mapping: connection exception -> ProviderConnectionFailed;
   ...
   (nonnegative delta-seconds or HTTP date via injected clock converted to
   milliseconds; absent/invalid/past -> 0); 500-599 -> ProviderServerError.
...
10. `php artisan agent:inspect-streaming-http` prints ONLY: resolved handler
    name ("stream"), whether http/https stream wrappers exist, allow_url_fopen
    boolean, validated connect/read/total seconds, pass/fail verdict; exit 0
