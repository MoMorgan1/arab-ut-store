...
- Made Home/Coins navigation react to cold direct hashes, clicks, Back, and Popstate/Hashchange updates.
...
- Added three pointer-inert, screen-reader-hidden floating coin decorations behind hero content, with restrained float/rotation and all motion disabled under `prefers-reduced-motion: reduce`.
...
- Review RED: a cold `/en#coins` render left Home current until an event was dispatched. The `useSyncExternalStore` fix makes the initial browser snapshot authoritative while retaining an SSR-safe server snapshot.

## Automated verification

- Focused Pest: 6/6 passed, 552 assertions.
- Focused Vitest after review fixes: 67/67 passed.
- Full `npm run ci:check` after review fixes: 195/195 tests passed; ESLint, Prettier, TypeScript, and Vite build passed.
- `git diff --check`: passed.

## Browser verification

Local verification used the worktree `public` directory with Laravel's absolute `server.php` router on `127.0.0.1:8017` and the project PHP configuration.
...
- Navigation: cold direct `#coins`, click, and Back each produced the correct single `aria-current` item.
- Preferences: attribution link was visible, keyboard-focusable, pointed to `https://www.exchangerate-api.com`, and no provider link remained in the footer.
- Motion sampling showed a restrained rotation delta around each coin's base orientation; reduced-motion emulation computed all coin animations to `none` while retaining their base rotations.
