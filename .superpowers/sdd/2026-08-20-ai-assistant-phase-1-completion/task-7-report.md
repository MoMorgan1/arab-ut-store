...
| `npm run test:e2e`                                                  | Passed: exit 0; 7 Chromium tests passed in 42.2 seconds, including the authenticated account test in 27.7 seconds.                                                                                                                                                                         |
...
| `php C:\temp\arabut-composer\composer.phar ci:check`                | Passed: exit 0. Composer validation, config-cache clear, Pint, PHPStan (0 errors), Pest (851 tests, 33,142 assertions), Vitest (52 files, 369 tests), ESLint, Prettier, TypeScript, E2E TypeScript, and Vite build passed; only existing runtime-resolved asset/font notices were emitted. |
...
- The controller-run exact E2E gate passed: `npm run test:e2e` exited 0 with all 7 Chromium tests passing. This is local browser evidence only.
- The controller-run Composer gate passed: `php C:\temp\arabut-composer\composer.phar ci:check` exited 0. MariaDB CI, production, device, and owner-acceptance evidence remain separate pending gates.
...
| E1 — literal PowerShell evidence-boundary script below                                                                                                                                                                  | Passed: local Composer/E2E exits are asserted alongside MariaDB, partial F06, persistent SQLite fixture, and rollback/schedule boundaries.                                                                                                                                                 |
...
if($status -match 'Complete `php C:\\temp\\arabut-composer\\composer.phar ci:check`.*Passed locally: exit 0' -and $status -match 'Complete `npm run test:e2e`.*Passed locally: exit 0' -and $status -match 'MariaDB verification' -and $status -match 'partially addressed'){'STATUS evidence-boundary scan: PASS'}else{throw 'STATUS evidence boundary is incomplete'}
...
if($operations -match 'release applied a migration batch' -and $operations -match 'refuses to reactivate older code' -and $operations -match 'withoutOverlapping\(\)' -and $runbook -match 'release with no migration changes restores the prior symlinks directly' -and $runbook -match 'applied migrations first runs `php artisan migrate:rollback --force`' -and $runbook -match 'If schema rollback fails, older code is not restored' -and $runbook -match 'first deployment or missing/invalid prior release'){'Operations rollback/schedule scan: PASS'}else{throw 'Operations rollback/schedule claim is incomplete'}
```
...
## Controller-run exact local gates — 2026-08-21

The following fresh command-scoped evidence supersedes the earlier incomplete
local Composer/E2E capture. It does not supersede pending MariaDB CI,
production-session, deployment, production-health, iPhone/Safari, or Mohamed
acceptance gates.

| Exact command                                        | Exit and concise result                                                                                                                                                                                                                                                                              |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `php C:\temp\arabut-composer\composer.phar ci:check` | Exit 0. Composer validation, configuration-cache clear, Pint, PHPStan (0 errors), Pest (851 tests, 33,142 assertions), Vitest (52 files, 369 tests), ESLint, Prettier, TypeScript, E2E TypeScript, and Vite production build passed. Only existing runtime-resolved asset/font notices were emitted. |
| `npm run test:e2e`                                   | Exit 0. All 7 Chromium tests passed in 42.2 seconds; the authenticated account test passed in 27.7 seconds.                                                                                                                                                                                          |
