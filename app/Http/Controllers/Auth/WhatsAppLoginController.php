...
final class WhatsAppLoginController extends Controller
{
    ...
    public function verify(VerifyWhatsAppCodeRequest $request, VerifyWhatsAppLoginCode $verify): JsonResponse
    {
        $phone = $this->phone((string) $request->validated('phone'));

        try {
            $user = $verify->execute($phone, (string) $request->validated('code'));
        } catch (DomainException) {
