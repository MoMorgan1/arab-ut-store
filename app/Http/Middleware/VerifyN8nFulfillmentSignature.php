...
final class VerifyN8nFulfillmentSignature
{
    ...
    private function signatureMatches(Request $request): bool
    {
        ...
        $configuredKey = Config::get('services.n8n.fulfillment_key');
        $configuredSecret = Config::get('services.n8n.fulfillment_secret');

        // Anything shorter than 32 characters is a half-configured credential
        // rather than a weak one: comparing against it would authenticate
        // requests signed with a value too small to be a real HMAC key.
        if (! is_string($configuredKey)
            || $configuredKey === ''
            || ! is_string($configuredSecret)
            || strlen($configuredSecret) < 32
    ...
    private function unauthorized(): JsonResponse
    {
        return response()->json([
            'error' => ['code' => 'invalid_signature', 'message' => 'The fulfillment request signature is invalid.'],
    ...
    {
        return response()->json([
            'error' => ['code' => 'stale_placement', 'message' => 'The fulfillment request is outside the freshness window.'],
