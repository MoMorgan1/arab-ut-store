...
function Stop-Usage {
    ...
    [Console]::Error.WriteLine("codex-worker: $Message")
    exit 2
}
...
$relayPath = Join-Path $repoRoot '.agents\skills\codex-delegate\scripts\relay.mjs'
...
if (-not [System.IO.File]::Exists($relayPath)) {
    Stop-Usage 'The pinned project-local codex-delegate relay is missing.'
}
...
foreach ($heading in @('Objective', 'Allowed paths', 'Non-goals', 'Acceptance criteria', 'Required checks')) {
...
$temporaryBrief = Join-Path ([System.IO.Path]::GetTempPath()) "arabut-codex-brief-$([guid]::NewGuid()).md"
...
Never run git add, commit, push, pull, fetch, reset, restore, checkout, switch,
rebase, merge, cherry-pick, clean, stash, branch, or worktree commands. Never
...
if ($gitHistoryChanged) {
    [Console]::Error.WriteLine('codex-worker: HIGH SEVERITY - Git history or refs changed during the worker run. No automatic recovery was attempted.')
    exit 70
}

if ($ReadOnly -and $readOnlyFilesChanged) {
    [Console]::Error.WriteLine('codex-worker: HIGH SEVERITY - read-only worker changed repository files. No automatic recovery was attempted.')
    exit 71
}
