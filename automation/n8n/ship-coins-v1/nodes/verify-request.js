...
if (missingEnv.length) {
    throw new Error(
        `[verify] missing n8n environment variable(s): ${missingEnv.join(', ')}`,
    );
}
if (String($env.N8N_ORDER_PAID_SECRET).length < 32) {
...
// file id instead, which the helper below resolves when it exists.
...
if (!equal(key, $env.N8N_ORDER_PAID_KEY)) {
...
const expected = crypto
    .createHmac('sha256', $env.N8N_ORDER_PAID_SECRET)
    .update(`${timestamp}\n${eventId}\n${raw}`)
    .digest('hex');
