...
test('Coins cart fingerprints are keyed canonical hashes of the owner and every request field', function () {
    $validated = [
    ...
    expect(CoinsCartFingerprint::generate('user:17', $validated, 'synthetic-application-key'))->toBe($expected)
        ->and(CoinsCartFingerprint::generate('guest:'.str_repeat('a', 64), $validated, 'synthetic-application-key'))
        ->not->toBe($expected)
