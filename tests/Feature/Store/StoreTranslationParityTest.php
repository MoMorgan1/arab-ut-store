...
test('Arabic and English shell translation leaves and placeholders stay in parity', function () {
    ...
    expect(data_get($arabic, 'header.fut_champions'))->toBe('فوت تشامبيونز')
        ...
        ->and(data_get($arabic, 'footer.payment_methods'))->toBe('طرق الدفع')
        ->and(data_get($english, 'footer.payment_methods'))->toBe('Payment methods')
...
test('the catalog exposes the refined SBC hierarchy and add feedback in both locales', function () {
    ...
    expect(data_get($arabic, 'services.sbc.page_title'))->toBe('حل تحديات بناء التشكيلة SBC')
        ...
        ->and(data_get($english, 'catalog.included'))->toBe('Coins + completion')
        ->and(data_get($english, 'catalog.assurance_no_players'))->toBe('your club is safe')
        ...
        ->and(data_get($english, 'product.sbc.selected'))->toBe('platform')
        ->and(data_get($english, 'product.sbc.related_title'))->toBe('You may also like')
