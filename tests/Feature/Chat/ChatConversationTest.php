...
test('a session pointer to a closed conversation is ignored when acquiring a conversation', function () {
    ...
    $response->assertOk();
    expect($response->json('data.publicId'))->not->toBe($closedConversation->public_id);
});
