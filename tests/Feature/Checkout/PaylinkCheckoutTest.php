...
function fakePaylinkCheckout(?string $orderNumber = null, string $getStatus = 'Pending'): void
{
    ...
    Cache::flush();
    Http::fake(function ($request) use ($getStatus, $orderNumber) {
...
test('checkout requires authentication and never creates a guest order', function () {
    ...
    $this->postJson('/checkout/paylink', [], ['Idempotency-Key' => 'checkout-http-guest'])
        ->assertUnauthorized()
...
test('a verified customer gets one safe Paylink URL and exact retries reuse the invoice', function () {
    ...
    $created = $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-created',
    ]);
    $replayed = $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-created',
    ]);
...
test('checkout returns stable errors for phone verification and changed carts', function () {
    ...
    $this->actingAs($unverified)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-phone',
    ])->assertUnprocessable()->assertJsonPath('error.code', 'phone_verification_required');

    ['user' => $user, 'variant' => $variant] = paylinkCheckoutCart();
    $variant->update(['price_halalah' => 1300, 'price_version' => 9]);

    $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-stale',
    ])->assertUnprocessable()->assertJsonPath('error.code', 'cart_changed');
});
...
test('a Paylink outage preserves the pending order for a safe retry', function () {
    ...
    $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-recover',
...
test('a retry whose Paylink invoice is already paid returns the order instead of reopening payment', function () {
    ...
    $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-paid-retry',
...
test('only the pending order owner can resume its Paylink payment', function () {
    ...
    fakePaylinkCheckout();
    $this->actingAs($owner)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-owner',
    ])->assertCreated();
...
test('the Paylink return verifies the invoice before marking the owner order received', function () {
    ...
    $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-paid',
    ])->assertCreated();
...
test('the authenticated Paylink webhook verifies the invoice before acknowledging payment', function () {
    ...
    $this->actingAs($user)->postJson('/checkout/paylink', [], [
        'Idempotency-Key' => 'checkout-http-webhook',
    ])->assertCreated();
