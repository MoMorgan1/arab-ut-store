...
**Final whole-branch review closure (2026-08-10):** The MariaDB workflow now exercises the five cart/security suites, including the real concurrency contract, and both `mariadb` and `mysql` driver names enter the locking/schema assertions. Guest Coins resume, email/password auth links and forms, password-reset URLs, and successful reset redirects preserve the originating Arabic or English storefront. The four approved payment badges are unchanged while the label now says `طرق الدفع عند الإطلاق` / `Payment methods at launch`. The obsolete `simple_pages.cart` placeholder contract is removed; the authenticated `/cart` and `/en/cart` pages remain real routes. Final SQLite CI passed 303 tests (300 passed, 3 skipped) and 3,042 assertions; frontend CI passed 12 files/144 tests; the exact focused MariaDB run passed 60 with 3 expected skips and 585 assertions. Browser verification passed Arabic and English auth at 320px and 807px with localized navigation/form actions, correct direction, Thmanyah typography, 44px effective targets, no overflow, and no console warning/error. Exact RED/GREEN and guard evidence is recorded in `.superpowers/sdd/2026-08-10-wordpress-header-footer-parity/final-fix-report.md`.

Historical note: the earlier code and translation snippets in this implementation plan document the then-current simple-page placeholder and payment-copy contract. They are intentionally retained as historical planning evidence and are superseded by the live contracts and final closure evidence above.
