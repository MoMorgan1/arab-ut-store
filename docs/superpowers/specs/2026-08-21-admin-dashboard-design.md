...
- On 2026-08-23, Mohamed decided: "MFA doesn't need a page — we need a Settings page." The standalone MFA Security page becomes the Security section of a new Settings page. Navigation becomes Overview · Orders · Customers · Settings (the "MFA Security" item disappears). Settings v1 contains exactly two sections: Security (the existing MFA enrolment/recovery-codes UI, for every Admin/Staff actor) and Team (Admin-only staff management: role promotion/demotion, activation/deactivation, last-active-admin guard, 409 stale conflict handling, session revocation on deactivate). Adding a brand-new staff member stays the console command (`admin:grant-role`) in v1.
