...
| `correctCredentialsAPI` | credential fix **and resume**, for a **coins** order | Not two endpoints. Resume is the same call with `continue: 1`; the tracker's `continueFftOrder()` posts here. Keyed by `orderID`; `user`, `pass` and the backup codes are ordinary optional fields beside it, so a corrected **email lands here** (`api-handlers.php:1178-1200`). |
...
| `createCustomerAPI` | rewrite the customer record — the credential fix for a **challenge** | Called with `updateCustomer: '1'`. Keyed by **account email**, so its effect reaches every order on that EA account at FFT, not just the one in hand. A challenge solve is bound to a customer record rather than to an order, which is why its correction goes here and not to `correctCredentialsAPI` (`api-handlers.php:2118-2137`). **Consequence: a changed email writes a different record while the solve stays on the original account, and the tracker treats `action: 'created'` as success — so it reports success and nothing moves.** Verify before shipping an editable email on a challenge item; see slice D1. Recorded in `docs/decisions/2026-09-12-ea-credentials-in-placement-payload.md`. |
